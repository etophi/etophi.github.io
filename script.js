...
if (loginForm) {
  loginForm.addEventListener("submit", function (e) {
    ...
    if (user === defaultUser && pass === defaultPass) {
      ...
      if (msg) msg.textContent = "⚠️ Username atau password salah!";
    }
  });
}

/* ================================
   CEK LOGIN UNTUK ADMIN PAGES
================================= */
...
function renderStats() {
  const stats = JSON.parse(localStorage.getItem("siteStats")) || { visits: 0, pages: {} };
  const total = document.getElementById("total-visits");
  if (total) total.textContent = stats.visits;
